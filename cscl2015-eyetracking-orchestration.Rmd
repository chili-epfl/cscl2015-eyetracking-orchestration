...
We uncompress the data from the study (warning, it is almost 1G of files!). This includes mainly a time series of eyetracking data along with the value of several game variables (e.g., mean Tetris stack height) in each instant, plus an additional set of files with the fixation details of each game in the experiment.
...
cleandata <- preprocessStudy1()
```

### Data analysis

The main basic data analysis we do here is just to calculate the Load Index for each 10s window in a game:

```{r}
source("../lib/loadIndex.R")

# We load the overall dataset with the data from the 10s sliding windows
totaldata <- get(load(cleandata))
